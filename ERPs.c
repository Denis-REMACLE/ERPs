...
void search_pokemon(pokemon_t* fullpokedex, pokemon_t* userpokedex){
    ...
    int is_known = 0;
    int rarity_to_use, i, option, luck;
    int poke[10];
...
void create_pokemon(pokemon_t* fullpokedex, pokemon_t* userpokedex){
    printf("[%d] :\t%s\n", fullpokedex->number, fullpokedex->name);
    exit(0);
}

void update_pokemon(pokemon_t* fullpokedex, pokemon_t* userpokedex){
    printf("[%d] :\t%s\n", fullpokedex->number, fullpokedex->name);
    exit(0);
}

void delete_pokemon(pokemon_t* fullpokedex, pokemon_t* userpokedex){
    printf("[%d] :\t%s\n", fullpokedex->number, fullpokedex->name);
    exit(0);
}

void edit_full_pokedex(pokemon_t* fullpokedex){
    void (*array_fptr[6])(pokemon_t* fullpokedex, pokemon_t* userpokedex);
    int option;

    SAVE = "full_pokedex.csv";
    ...
    while (1 == 1){
        ...
        array_fptr[1] = &read_pokedex;
        array_fptr[2] = &update_pokemon;
        array_fptr[3] = &delete_pokemon;
        array_fptr[4] = &save_pokedex;
        array_fptr[5] = &quit;

        array_fptr[option](fullpokedex, fullpokedex);
    }
    exit(0);
}
